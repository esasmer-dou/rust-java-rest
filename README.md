...
[![Version](https://img.shields.io/badge/version-4.5.4-blue.svg)](https://github.com/esasmer-dou/rust-java-rest/releases/tag/v4.5.4)
...
[![Status](https://img.shields.io/badge/status-stable-blue.svg)](https://github.com/esasmer-dou/rust-java-rest/releases/tag/v4.5.4)
...
`4.5.4` is the stable declarative runtime line. It packages clean `rust-spring v4.5.4` native
...
When bounded telemetry is enabled, `4.5.4` stores its per-request capture state in one compact
...
`v4.5.4` keeps the Java handler and service model unchanged. It retains the bounded telemetry
...
logic remain source-compatible. Deploy the ABI `29` DLL/SO packaged with the coordinated `4.5.4`
artifact.
...
java -jar sample/target/rust-java-rest-4.5.4-sample.jar
```
...
The stable `4.5.4` line can use `java-rust-glowroot-agent:0.3.0`. It sends bounded
...
with no telemetry thread. Version `4.5.4` isolates export and profile reclamation on one `256 KiB`
